...
func init() {
	android.RegisterModuleType("llvm18_min_tblgen", llvm18MinTblgenFactory)
}

var (
	min_tblgen_pctx = android.NewPackageContext("android/soong/llvm18_min_tblgen")

	llvm18MinTblgen = min_tblgen_pctx.HostBinToolVariable("llvm18MinTblgen", "llvm18-min-tblgen")

	min_tblgenRule18 = min_tblgen_pctx.StaticRule("min_tblgenRule18", blueprint.RuleParams{
		Depfile:     "${out}.d",
		Deps:        blueprint.DepsGCC,
		Command:     "${llvm18MinTblgen} ${includes} ${generator} -d ${depfile} -o ${out} ${in}",
		CommandDeps: []string{"${llvm18MinTblgen}"},
		Description: "LLVM18 Min TableGen $in => $out",
		Restat:      true,
...
func llvm18MinTblgenFactory() android.Module {
	t := &min_tblgen{}
