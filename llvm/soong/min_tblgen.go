...
func init() {
	android.RegisterModuleType("llvm20_min_tblgen", llvm20MinTblgenFactory)
}

var (
	min_tblgen_pctx = android.NewPackageContext("android/soong/llvm20_min_tblgen")

	llvm20MinTblgen = min_tblgen_pctx.HostBinToolVariable("llvm20MinTblgen", "llvm20-min-tblgen")

	min_tblgenRule20 = min_tblgen_pctx.StaticRule("min_tblgenRule20", blueprint.RuleParams{
		Depfile:     "${out}.d",
		Deps:        blueprint.DepsGCC,
		Command:     "${llvm20MinTblgen} ${includes} ${generator} -d ${depfile} -o ${out} ${in}",
		CommandDeps: []string{"${llvm20MinTblgen}"},
		Description: "LLVM20 Min TableGen $in => $out",
		Restat:      true,
...
func llvm20MinTblgenFactory() android.Module {
	t := &min_tblgen{}
