...
func init() {
	android.RegisterModuleType("llvm17_min_tblgen", llvm17MinTblgenFactory)
}

var (
	min_tblgen_pctx = android.NewPackageContext("android/soong/llvm17_min_tblgen")

	llvm17MinTblgen = min_tblgen_pctx.HostBinToolVariable("llvm17MinTblgen", "llvm17-min-tblgen")

	min_tblgenRule17 = min_tblgen_pctx.StaticRule("min_tblgenRule17", blueprint.RuleParams{
		Depfile:     "${out}.d",
		Deps:        blueprint.DepsGCC,
		Command:     "${llvm17MinTblgen} ${includes} ${generator} -d ${depfile} -o ${out} ${in}",
		CommandDeps: []string{"${llvm17MinTblgen}"},
		Description: "LLVM17 Min TableGen $in => $out",
		Restat:      true,
...
func llvm17MinTblgenFactory() android.Module {
	t := &min_tblgen{}
